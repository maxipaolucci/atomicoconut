...
const create = async (propertyId, data, userEmail, res) => {
    ...
    console.log(`${methodTrace} ${getMessage('message', 1031, userEmail, true, 'PropertyAdditionalInfo')}`);
    data.parent = propertyId;
    let propertyAdditionalInfo = await (new PropertyAdditionalInfo(data)).save();

    if (!propertyAdditionalInfo) {
        console.log(`${methodTrace} ${getMessage('error', 459, userEmail, true, 'PropertyAdditionalInfo')}`);
        res.status(401).json({ 
            status : "error", 
            codeno : 459,
            msg : getMessage('error', 459, null, false, 'PropertyAdditionalInfo'),
            data : null
        });
    ...
    console.log(`${methodTrace} ${getMessage('message', 1026, userEmail, true, 'PropertyAdditionalInfo')}`);
...
exports.update = async (propertyId, data, userEmail, res) => {
    ...
    console.log(`${methodTrace} ${getMessage('message', 1024, userEmail, true, 'PropertyAdditionalInfo', '_id', propertyAdditionalInfo._id)}`);
    ...
    if (!propertyAdditionalInfo) {
        ...
        console.log(`${methodTrace} ${getMessage('error', 465, userEmail, true, 'PropertyAdditionalInfo', '_id', propertyAdditionalInfo._id)}`);
        res.status(401).json({ 
            status : "error", 
            codeno : 465,
            msg : getMessage('error', 465, null, false, 'PropertyAdditionalInfo', '_id', propertyAdditionalInfo._id),
            data : null
        });
    }
    
    //success
    console.log(`${methodTrace} ${getMessage('message', 1028, userEmail, true, 'PropertyAdditionalInfo')}`);
    return propertyAdditionalInfo._id;
};
...
exports.delete = async (propertyId, userEmail, res) => {
    ...
    if (!(writeResult && writeResult.n > 0)) {
        ...
        console.log(`${methodTrace} ${getMessage('error', 464, userEmail, true, 'PropertyAdditionalInfo', 'parent', propertyId)}`);
        res.status(401).json({ 
            status : "error", 
            codeno : 464,
            msg : getMessage('error', 464, null, false, 'PropertyAdditionalInfo', 'parent', propertyId),
            data : null
        });
    ...
    console.log(`${methodTrace} ${getMessage('message', 1039, userEmail, true, 'PropertyAdditionalInfo')}`);
    return true;
};
...
const getByIdObject = async (propertyId, userEmail = null, options = null) => {
    ...
    console.log(`${methodTrace} ${getMessage('message', 1034, userEmail, true, 'PropertyAdditionalInfo', 'parent', propertyId)}`); 
    try {
    ...
    console.log(`${methodTrace} ${getMessage('message', 1036, userEmail, true, result ? 1 : 0, 'PropertyAdditionalInfo(s)')}`);
    return result;
};
