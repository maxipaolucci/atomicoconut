...
if [[ "$CURRENT_BRANCH" == "master" || "$CURRENT_BRANCH" == "testing" ]]; then
  ...
  docker build -t $DOCKER_ID/atomic-coconut-filebeat:$IMAGE_TAG -f ./elk/Dockerfile.filebeat ./elk
  ...
  docker push $DOCKER_ID/atomic-coconut-filebeat:$IMAGE_TAG
  ...
  sed -i "s?atomic-coconut-filebeat?atomic-coconut-filebeat:$IMAGE_TAG?" docker-compose-beanstalk.yml
else
  echo
